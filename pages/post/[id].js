...
export default function Post({ post }) {
  // each post will be passed in here
  // get reference to current signed in account using context
  const account = useContext(AccountContext)
  // get reference to router
  const router = useRouter()
  // router.query reads id
  const { id } = router.query

  // fallback route from next.js
...
export async function getStaticProps({ params }) {
  // pass the fetched posts and create a single page for each post
